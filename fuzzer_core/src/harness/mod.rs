...
static TARGET_MODE: OnceLock<TargetMode> = OnceLock::new();

fn target_mode() -> TargetMode {
    *TARGET_MODE.get_or_init(|| match std::env::var("IFB_TARGET").as_deref() {
        Ok("dummy") => TargetMode::Dummy,
        _ => TargetMode::Curl,
    })
}
...
pub fn fuzz_iteration(input: &[u8]) -> bool {
    match target_mode() {
        TargetMode::Curl => fuzz_curl(input),
        TargetMode::Dummy => fuzz_dummy(input),
    }
}

fn fuzz_curl(input: &[u8]) -> bool {
...
fn fuzz_dummy(input: &[u8]) -> bool {
    let mut score = 0u8;
    for byte in input.iter().take(64) {
        score = score.wrapping_add(*byte);
        if *byte == b'%' || *byte == b'?' {
    ...
    if input.ends_with(b"/") && input.contains(&b'.') {
